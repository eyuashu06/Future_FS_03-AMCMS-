...
export default function Contact() {
  ...
  const contactSchema = z.object({
    name: z.string().min(2, t('contact.reqNameLength')),
    email: z.string().email(t('contact.reqEmailVal')),
    phone: z.string().optional(),
    message: z.string().min(10, t('contact.reqMessageLength')),
  });
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      toast.success(t('contact.msgSuccessSent'));
      reset();
    } catch (error) {
      toast.error(error.message || t('contact.msgFailedSent'));
    } finally {
    ...
    <div className="min-h-screen">
      ...
      <section className="relative h-[400px] flex items-center justify-center overflow-hidden">
        ...
        <div className="container relative z-10 text-center text-white">
          <h1 className="text-4xl md:text-6xl font-bold mb-4 tracking-tight">{t('contact.contactUs')}</h1>
        ...
        <div className="container">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 max-w-6xl mx-auto">
            ...
            <div className="space-y-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="medical-card border-none bg-muted/50 p-6">
                  ...
                  <h3 className="font-bold text-lg mb-2">{t('contact.hospitalMch')}</h3>
                ...
                <div className="medical-card border-none bg-muted/50 p-6">
                  ...
                  <h3 className="font-bold text-lg mb-2">{t('contact.mainOffice')}</h3>
                  <div className="space-y-1 text-sm text-muted-foreground">
                    <p>+251 118 694 629</p>
                    <p>+251 111 553 622</p>
                  </div>
              ...
              <div className="medical-card border-l-4 border-l-accent p-8 bg-accent/5">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h3 className="text-xl font-bold">{t('contact.clinicalHours')}</h3>
                </div>
                <div className="grid grid-cols-2 gap-4 text-sm font-medium">
                  <div className="text-muted-foreground uppercase">{t('contact.emergencyDelivery')}</div>
                  <div className="text-accent text-right">{t('contact.twentyFourHours')}</div>
                  <div className="text-muted-foreground uppercase">{t('contact.genOpd')}</div>
                  <div className="text-foreground text-right">{t('footer.hoursWeekdays')}</div>
                  <div className="text-muted-foreground uppercase">{t('contact.laboratory')}</div>
                  <div className="text-foreground text-right">{t('footer.hoursWeekend')}</div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-card border border-border p-10 rounded-3xl shadow-xl shadow-muted/20">
              <h2 className="text-2xl font-bold mb-8">{t('contact.sendUsMessage')}</h2>
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <Label htmlFor="name">{t("contact.fullNameLabel")}</Label>
                    <Input id="name" {...register('name')} placeholder={t("contact.fullNamePlaceholder")} />
                ...
                <div className="space-y-2">
                  ...
                  <Textarea id="message" {...register('message')} placeholder={t("contact.messagePlaceholder")} rows={6} className="resize-none" />
                  {errors.message && <p className="text-xs text-destructive">{errors.message.message}</p>}
                </div>
                <Button type="submit" disabled={isSubmitting} className="w-full h-14 text-lg font-bold cta-button">
                  {isSubmitting ? t('contact.sendingMessage') : t('contact.submitInquiry')} <Send size={20} className="ml-2" />
                </Button>
              </form>
            </div>
          </div>
      ...
      <section className="py-20 bg-muted/30">
        <div className="container">
          <div className="flex flex-col items-center text-center mb-10">
            ...
            <h2 className="text-3xl font-bold mb-4">{t('contact.mapTitle')}</h2>
